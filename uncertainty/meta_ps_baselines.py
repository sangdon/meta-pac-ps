...
class PredSetConstructor_meta_ideal(PredSetConstructor_meta):
    ...
    def test(self, ld, ld_name, verbose=False):
        ...
        for x, y in ld:
            ...
            # split cal and test
            if type(x) is dict and 'x_adapt' in x:
            ...
            elif type(x[0]) is dict and 'word' in x[0]:
                ...
                assert(all([v.shape[-1] == dim for k, v in query.items() if k not in ['n_ways', 'n_shots']]))


                support = {k: v.view(-1, n_ways, n_shots_adapt, dim) for k, v in support.items()}
                query = {k: v.view(-1, n_ways*n_shots_test, dim) for k, v in query.items() if k not in ['n_ways', 'n_shots']}
